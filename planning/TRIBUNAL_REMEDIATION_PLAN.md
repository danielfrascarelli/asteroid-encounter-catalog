...
| 5 | Presupuesto de completitud honesto (σ elementos + injection-recovery) | B4 | ✅ MC de covarianzas SBDB (3000 draws/cuerpo, propagación 2-cuerpos) sobre los 17 pares de tablas extremas: σ_dist = 1-83 km (mediana 32), NO 10³-10⁴ (esos son numerados, órbitas multi-oposición). closest σ/d=2-8% → significativos; columna σ_d en tab:closest; §3/§4.2/§4.3 reescritos. injection-recovery previo | 1 | `scripts/validate/mc_distance_uncertainty.py`, §3 |
| 6 | σ de masas defendible (leverage, N mín, bootstrap) | B6 | ✅ bootstrap corrido (B=60) sobre los 4 measured con leverage>0.5; build_mass_catalog usa σ_boot para esos casos. Resultado: Ceres/Psyche confirmados (σ_boot≈σ_jack), Juno (0.93) y Thisbe (0.84) reclasificados measured→bound (σ_jack subestimaba 1.9-4×; snr_boot 2.4/1.8<3). tab:sixteen: 6 measured/8 bound | — | `src/orbdet/`, `mass_catalog` |
...
| 8 | Validación de calibradores no circular (LOCO) + inyección end-to-end | B8 | ✅ LOCO PASA (max \|z\|=1.40); inyección-recuperación del estimador (2e19→2.18e19, z=+0.5); ∂M/∂s_c minúsculo (∂lnM/∂ln s_c≈-0.01, ΔM<1.3% sobre s_c 0.5-1.1 mas para Ceres/Vesta/Hygiea) → piso infla la σ, no la masa central; validación no circular. Todo en §5 | 6 | `scripts/mass/`, `docs/` |
...
| 17 | Reencuadrar §5 como demo de uso; comparar con Siltala/FM | M8,M9 | ✅ framing demo de uso + Siltala/FM; tab:sixteen y cross-check FM RE-AJUSTADOS con targets b1fix (8 measured, 6 bound, 1 non-phys, 1 unconstr); chequeo de estabilidad reportado (calibradores <2%, partición measured/bound intacta salvo 1 en el borde snr=3) | 9,10 |
