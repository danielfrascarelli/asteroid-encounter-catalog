...
def _best_mpcorb_snapshot(archive_dir: Path, encounter_jd: float) -> Path:
    ...
    if best_path is None:
        raise FileNotFoundError(f"No valid MPCORB snapshots found in {archive_dir}")
    return best_path
...
def al_residuals_and_weights(
    ...
    var_al = _cov_al(ra_err_sys, dec_err_sys, corr_sys) + _cov_al(
        ra_err_rand, dec_err_rand, corr_rand
    )
...
def fit_orbit_loo(
    ...
    lo = np.array(
        [a0 - 0.05, max(0.0, e0 - 0.05), max(0.0, i0 - 2.0), O0 - 10.0, o0 - 10.0, M0 - 15.0]
    )
    hi = np.array([a0 + 0.05, min(0.999, e0 + 0.05), i0 + 2.0, O0 + 10.0, o0 + 10.0, M0 + 15.0])
    ...
    logger.info(
        "[LOO orbit] done: nfev=%d  status=%d  message=%s", res.nfev, res.status, res.message
    )
...
def fit_mass_from_window(
    ...
    shift_al = (
        float(np.mean(r_al0[post_mask]) - np.mean(r_al0[pre_mask]))
        if (n_pre > 0 and n_post > 0)
        else float("nan")
    )
    # Also keep 2D RMS for reference
    rms_pre_2d = (
        float(np.sqrt(np.mean(dra0[pre_mask] ** 2 + ddec0[pre_mask] ** 2)))
        if n_pre > 0
        else float("nan")
    )
    rms_post_2d = (
        float(np.sqrt(np.mean(dra0[post_mask] ** 2 + ddec0[post_mask] ** 2)))
        if n_post > 0
        else float("nan")
    )
    logger.info(
    ...
    if n_post < 3:
        return {
            "mass_kg": float("nan"),
            "mass_sigma_kg": float("nan"),
            "mass_sigma_inflated_kg": float("nan"),
            "log10_mass": float("nan"),
            "log10_mass_sigma": float("nan"),
    ...
    def residual_func(params: np.ndarray) -> np.ndarray:
        ...
        r_al, s_al = al_residuals_and_weights(
            ...
            ddec,
            pa_scan[post_mask],
            ra_err_sys[post_mask],
            dec_err_sys[post_mask],
            corr_sys[post_mask],
            ra_err_rand[post_mask],
            dec_err_rand[post_mask],
            corr_rand[post_mask],
        )
...
def main() -> int:
    ...
    if n_out < 8:
        logger.error(
            "Only %d out-of-window observations — need ≥ 8 to fit 6 orbital params.",
            n_out,
        )
        return 1
    ...
    logger.info(
        ...
        target_el["e"],
        args.perturber,
        perturber_el.get("H", float("nan")),
        initial_mass,
    )
    logger.info(
        "Background asteroids (%d/%d): %s",
        len(background_elements),
        background_n,
        list(background_elements.keys()),
    )
    ...
    logger.info(
        ...
        mass_result["rms_pre_al_mas"],
        mass_result["rms_post_al_mas"],
    )
    logger.info(
        "  AL shift (post−pre): %.2f mas  (2D pre=%.1f mas  post=%.1f mas)",
        mass_result["shift_al_mas"],
        mass_result["rms_pre_2d_mas"],
        mass_result["rms_post_2d_mas"],
    )
    if math.isfinite(mass_result["mass_kg"]):
        logger.info(
            "  Fitted mass = %.3e ± %.2e kg  (log10=%.2f ± %.2f)",
            mass_result["mass_kg"],
            mass_result["mass_sigma_kg"],
            mass_result["log10_mass"],
            mass_result["log10_mass_sigma"],
        )
    else:
