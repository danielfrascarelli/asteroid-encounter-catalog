...
def _make_al_args(
    ...
    return (
        dra, ddec, pa_deg,
        np.full(n, s_ra_sys), np.full(n, s_dec_sys), np.full(n, rho_sys),
        np.full(n, s_ra_rand), np.full(n, s_dec_rand), np.full(n, rho_rand),
    )
...
class TestSigmaAL:
    def test_floor_when_all_errors_zero(self):
        ...
        pa = np.zeros(n)
        zeros = np.zeros(n)
        _, sigma_al = al_residuals_and_weights(zeros, zeros, pa,
                                               zeros, zeros, zeros,
                                               zeros, zeros, zeros)
    ...
    def test_systematic_dominates(self):
        ...
        _, sigma_al = al_residuals_and_weights(
            np.zeros(n), np.zeros(n), pa,
            np.zeros(n), np.full(n, s_dec_sys), np.zeros(n),
    ...
    def test_combined_variances_add(self):
        ...
        s_rand = 4.0   # mas
        _, sigma_al = al_residuals_and_weights(
            np.zeros(1), np.zeros(1), pa,
            np.zeros(1), np.full(1, s_sys),  np.zeros(1),
            np.zeros(1), np.full(1, s_rand), np.zeros(1),
        )
    ...
    def test_correlation_increases_sigma(self):
        ...
        _, s_uncorr = al_residuals_and_weights(
            np.zeros(n), np.zeros(n), pa,
            np.full(n, s), np.full(n, s), np.zeros(n),
            np.zeros(n),   np.zeros(n),   np.zeros(n),
        )
        _, s_corr = al_residuals_and_weights(
            np.zeros(n), np.zeros(n), pa,
            np.full(n, s), np.full(n, s), np.full(n, 1.0),  # rho=+1
            np.zeros(n),   np.zeros(n),   np.zeros(n),
        )
