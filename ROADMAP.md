...
- **Cierre del `current_working_plan.md` (mayo 2026, PRs #53–#56)**: **A1** — recall del prefiltro medido (76.4 % en cola adversa; fix radial-overlap → 100 %; [docs/prefilter_recall.md](docs/prefilter_recall.md)). **A2** — caracterización por streaming del catálogo completo, `data/output/encounters_characterized_full.parquet` (72.236.904 filas, 18.9 % Gaia-observables, sin OOM). **B1** — validación de literatura consolidada ([docs/literature_validation.md](docs/literature_validation.md)): gate 4/4, Fienga 3/4, Galád 4/4, **Fuentes-Muñoz 2025 11.804/40.004 confirmaciones**; Goffin sin tabla de encuentros en VizieR. **B2** — dashboard memory-safe sobre el 72M + README final + reproducibilidad. (Las líneas de validación "100 % / 4/4 Fienga" de más arriba describen una corrida Kepler anterior; la verdad consolidada sobre el catálogo congelado es Fienga **3/4** — ver el doc.) Audit blocker #6 (refactor de masas) quedó primero **cerrado** (el LOO no era determinable en DR3) y luego **resuelto de verdad** por el motor `orbdet` (joint orbit+mass sobre FPR, junio 2026 — ver "Capa de masas" abajo); la caracterización del catálogo grande también queda cerrada (A2).
