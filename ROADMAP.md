...
- **Refinamiento N-body selectivo del catálogo (Track 1 Stage A/B, PRs #31/#32, mayo 2026)**: caracterización del error Kepler-vs-N-body sobre 964 pares estratificados (p99 \|Δdist\| = 2.5 mAU; el error escala con `e_max` y `1/q_min`) seguida de re-refinamiento N-body sobre el subset crítico `q_min < 1.8 ∨ e_max > 0.3` (8.728.509 / 72.236.904 pares; 12.08 %). Catálogo híbrido `data/output/encounters_catalog_hybrid_stageb.parquet` con `refinement_method ∈ {kepler, nbody}` por fila; 25.283 pares (0.29 % del subset refinado) cruzan el threshold hacia arriba al refinar — un efecto de **censura + scatter cerca del corte**, NO un sesgo unidireccional (caracterizado y corregido en [docs/kepler_threshold_bias_paper.md](docs/kepler_threshold_bias_paper.md); la tasa de falsos negativos no es medible con este catálogo). Refinamiento universal del catálogo entero (Stage C) descartado por costo (~400 días-CPU). Detalle en [docs/kepler_refine_error_report.md](docs/kepler_refine_error_report.md) y [FROZEN_RUN.md](FROZEN_RUN.md).
- **Cierre del `current_working_plan.md` (mayo 2026, PRs #53–#56)**: **A1** — recall del prefiltro medido (76.4 % en cola adversa; fix radial-overlap → 100 %; [docs/prefilter_recall.md](docs/prefilter_recall.md)). **A2** — caracterización por streaming del catálogo completo, `data/output/encounters_characterized_full.parquet` (72.236.904 filas, 18.9 % Gaia-observables, sin OOM). **B1** — validación de literatura consolidada ([docs/literature_validation.md](docs/literature_validation.md)): gate 4/4, Fienga 3/4, Galád 4/4, **Fuentes-Muñoz 2025 11.804/40.004 confirmaciones**; Goffin sin tabla de encuentros en VizieR. **B2** — dashboard memory-safe sobre el 72M + README final + reproducibilidad. (Las líneas de validación "100 % / 4/4 Fienga" de más arriba describen una corrida Kepler anterior; la verdad consolidada sobre el catálogo congelado es Fienga **3/4** — ver el doc.) Audit blocker #6 (refactor de masas) queda **resuelto por cierre** (la capa de masas no es determinable en DR3); la caracterización del catálogo grande también queda cerrada (A2).
- **Follow-up post-deepwork (mayo 2026, PRs #41–#51)**: cerró el plan `FOLLOWUP_PLAN.md` (disuelto; info consolidada aquí y en `docs/`). **Track A — capa de masas: CERRADA** con veredicto endurecido: *ninguna masa de perturbador es determinable con Gaia DR3 en este pipeline, incluido Ceres* (no-identificabilidad por degeneración M↔deltas activa sobre datos reales + leverage insuficiente; el mínimo de χ²(masa) de Ceres barre 0.04–4.37× lit según la ventana de ajuste, multimodal siempre). Detalle: [docs/mass_layer_track_a_closure.md](docs/mass_layer_track_a_closure.md). **Track B**: outliers de Stage 2 = misfit along-scan, no sistemática AC ([docs/mass_layer_stage2_outliers.md](docs/mass_layer_stage2_outliers.md)); specificity completo 0/27 detecciones (apéndice en [docs/mass_layer_stage3_diagnostic.md](docs/mass_layer_stage3_diagnostic.md)); sesgo del threshold Kepler caracterizado y corregido ([docs/kepler_threshold_bias_paper.md](docs/kepler_threshold_bias_paper.md)).
...
- **Capa de masas: NO publicable — Track A CERRADO definitivamente** (mayo 2026, PRs #34–#38 deepwork + #41–#51 follow-up). Veredicto final tras agotar las vías de rescate (priors apretados A1, multi-target joint A2, optimizador perfilado A2.5, diagnóstico real-data A2.6, gate-check A3): *ninguna masa de perturbador es determinable con Gaia DR3 en este pipeline, **incluido Ceres***. Mecanismo: **no-identificabilidad** — la degeneración masa↔deltas-orbitales está activa sobre datos reales, volviendo la χ²(masa) multimodal; el mínimo de Ceres (el de mayor leverage, σ~20) barre 0.04–4.37× literatura según la ventana de ajuste, sin plateau. La specificity completa da **0/27 detecciones** (`p_mass` nunca < 0.08). Cierre y experimento en [docs/mass_layer_track_a_closure.md](docs/mass_layer_track_a_closure.md); linaje completo en los `docs/mass_layer_stage_a*.md`.
  - **Implicación**: las masas previamente reportadas ((111) Ate 5.43×10¹⁷ kg, etc.) **no son defendibles**. La capa sólo se reabriría con **Gaia DR4 / FPR** (arcos más largos → más leverage); el tooling (profiled, multi-target, ventana acotada) queda listo.
