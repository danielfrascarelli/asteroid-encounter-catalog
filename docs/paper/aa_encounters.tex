...
\emph{widened} to $r_q = 0.05\,\mathrm{AU} + v_{\max}\,(\Delta t/2) \approx
0.0536$\,AU with $v_{\max} = 25$\,km\,s$^{-1}$: under near-linear relative motion a
...
refinement then discards any whose true minimum exceeds 0.05\,AU. The bracketing is
exact for relative velocities up to $v_{\max}$, and the residual (encounters with
