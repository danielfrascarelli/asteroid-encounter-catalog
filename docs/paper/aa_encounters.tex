...
\title{A systematic catalogue of real three-dimensional close encounters
between numbered asteroids during the Gaia DR3 window, with a measured
completeness budget}
...
determination and a dynamical observable of the main belt. Existing
encounter-based work selects individual events by hand, and no systematic
catalogue of real three-dimensional asteroid--asteroid encounters has had its
own completeness characterised.}
% aims
{We build a systematic catalogue of real 3D close encounters between numbered
asteroids during the Gaia DR3 window (2014 July 25 -- 2017 May 28) and, unlike a
...
approach, not a sky-plane co-location --- for 72\,236\,904 pairs within
...
12\,$\mu$AU, p99 2.5\,mAU), the threshold-induced false-negative rate
(0.70\,\%, symmetric and scatter-dominated), and the prefilter recall on the
adverse high-$e$/high-$i$ tail (76\,\%). Applied to mass determination, the
...
mass determination \citep{michalak2000, goffin2014, fuentesmunoz2024,
fuentesmunoz2025}. Encounters between members of the same dynamical family
probe the collisional history of the belt, and the statistical distribution of
mutual approach distances and velocities is itself a dynamical observable.

Existing work on encounter-based mass determination
\citep{goffin2014, fuentesmunoz2024, fuentesmunoz2025} starts from
\emph{hand-selected} individual encounters chosen for their favourable
geometry, typically a single large perturber against a short list of
well-observed test bodies. There is, to our knowledge, no systematic catalogue
of real 3D asteroid--asteroid encounters whose own completeness has been
characterised. That is the gap this work fills: we detect encounters
exhaustively over the full numbered population in the Gaia DR3 window, and ---
the point that separates this catalogue from a bulk propagation exercise --- we
\emph{measure} what the detection pipeline misses rather than asserting
completeness.
...
\citep{tanga2023}, 2014 July 25 -- 2017 May 28. All internal computation is
...
KD-tree ($O(N\log N)$ construction), and a radius query at the 0.05\,AU
threshold returns neighbour candidates ($O(\log N)$ per query), avoiding any
$O(N^2)$ distance matrix. The coarse cadence is $\Delta t = 12$\,h.
...
0.06\,AU threshold over 10\,000 numbered bodies and isolating the 17\,469 pairs
with Kepler distance in $[0.05, 0.06)$\,AU --- exactly the band a 0.05\,AU
...
instead bounded by the 12\,h KD-tree coarse cadence (fast minima can fall
between grid samples) and by the threshold censoring of
Sect.~\ref{sec:censoring}. For the dynamically cold bulk of the belt (low $e$,
...
As an external check we cross-matched the hand-selected (perturber, target)
...
approached within 0.05\,AU, because a measurable mass deflection does not
require so close an encounter and their signal-ranked target lists are therefore
not restricted to 0.05\,AU. A further 8.4\,\% are targets or perturbers outside
...
$\sim$36\,\%. Pair-level cross-matching against \citet{goffin2014} was not
possible: the available VizieR tables give per-perturber masses only, not the
individual encounter pairs.
...
\citet{fuentesmunoz2025} already published 231 masses covering essentially all
classical large MBAs. The novelty of this work is the dataset itself --- a
catalogue with a measured completeness budget --- and the methodological
framework of Sect.~\ref{sec:mass}, not a single spectacular event.

% =========================================================================
\section{Application to mass determination}
\label{sec:mass}

The catalogue is the target-selection front-end of an asteroid
mass-determination engine. Perturber mass is obtained by a joint least-squares
...
Gaia FPR astrometry. The force model is ASSIST (JPL DE440 ephemeris; EIH
relativistic corrections; sixteen massive asteroid perturbers); the
implementation is our own, with no third-party orbit-determination dependency.
...
with \citealt{fuentesmunoz2025} at $|z|<3$ under a jackknife $\sigma$), and
generalises to bodies beyond the ephemeris set. We are explicit about the
...
are already well surveyed. The contribution is the dataset with its quantified
budget and the methodological framework (FOV-block covariance, identifiability
criterion, external jackknife $\sigma$), which together turn a bulk propagation
into a defensibly characterised catalogue.
