...
fits sixteen perturbers, eight of which are identifiable; these are broadly
...
the mass$\leftrightarrow$orbit regression (Sect.~below) and either fall to bounds
or, as with (88) Thisbe (FM mass $8.8\times10^{18}$\,kg, our fit $6.9\times
10^{18}$\,kg), survive a naive $\mathrm{snr_{jack}}$ flag only as a
regression-suppressed estimate. The mass layer does
...
(Table~\ref{tab:sixteen}); eight clear the identifiability criterion below and are
flagged \emph{measured}, six are \emph{bounds}, one (Davida, $N=6$) is
...
($\sigma_{\rm tot} = 8$\,\%): ratio 0.95 vs its DE441 seed and 0.95 ($z=-0.68$) vs
...
(3) Juno         & 24 & $2.0\times10^{19}$  & 23\,\%  & 0.99 & 0.70 & $-1.9$ & 0.93 & measured \\
...
(16) Psyche      & 26 & $2.3\times10^{19}$  & 8\,\%   & 0.98 & 0.95 & $-0.6$ & 0.64 & measured \\
...
(88) Thisbe      & 27 & $6.9\times10^{18}$  & 15\,\%  & 0.98 & 0.39 & $-10.6$ & 0.84 & measured \\
...
resolve deviations of the size the fits themselves show. Of the five measured
non-calibrators, all five lie \emph{below} the FM value with a geometric-mean ratio
of 0.82 --- a coherent $\sim$18\,\% one-sided pull (the regression-toward-zero
signature; a sign test gives $p\approx0.06$, suggestive but not significant at
...
$\sigma_{\rm jack}$ rests on effectively one replica; for these we also compute a
nonparametric bootstrap $\sigma$ (resampling targets with replacement, warm-started
from the converged fit), which is robust to that single-replica dominance because a
high-leverage target does not appear in every resample.
