...
fits sixteen perturbers, six of which are identifiable; these are broadly
...
the resulting mutual distance. The propagated $1\sigma$ is small --- 1--83\,km
(median $\approx32$\,km) over the $\pm1.4$\,yr window --- because the catalogue's
universe is \emph{numbered} asteroids, whose multi-opposition orbits are secure;
the $10^3$--$10^4$\,km one would fear applies to short-arc objects, which are not
in the sample. This term does not affect the completeness \emph{fractions} below
(those are pipeline-internal), and it now sets, rather than merely bounds, the
physical significance of individual tabulated distances (Sect.~\ref{sec:extreme},
Sect.~\ref{sec:family}): the closest approaches are significant to $\sim$2--8\,\%
and the wider extreme events to $\ll1$\,\%. These are formal SBDB covariances,
which can be optimistic by a factor of a few for the faintest bodies; even so the
conclusion (tens of km, not $10^3$--$10^4$) is unchanged. Each measured term is
...
0.06\,AU threshold over 10\,000 numbered bodies --- a \emph{stratified} sample
drawn to span the belt's $(a, e, i)$ range rather than a density-weighted random
draw, so the crossing rate estimates the effect uniformly across the belt rather
than its population-weighted mean --- and isolating
...
separations of $\sim$1000--2000\,km (Table~\ref{tab:closest}). We propagated each
body's JPL SBDB covariance by Monte Carlo (Sect.~\ref{sec:completeness}) to attach
an input-element $1\sigma$ to these distances (last column): they are $32$--$83$\,km,
i.e.\ $\sim$2--8\,\% of the separation, so the closest approaches are significant
well below their own value --- not the order-of-magnitude figures a short-arc
population would give. The two remaining caveats are that these are formal
covariances (optimistic by up to a few$\times$ for the faintest bodies) and that
two-body propagation is most fragile at the smallest separations, so N-body
revalidation (Sect.~\ref{sec:refineerror}) is still advisable before any individual
approach is claimed as physical.
...
\begin{table*}
\caption{The five closest approaches in the catalogue. $\sigma_d$ is the
input-element $1\sigma$ on the separation, from a Monte-Carlo propagation of each
body's JPL SBDB covariance under the catalogue's two-body model
(Sect.~\ref{sec:completeness}); at 2--8\,\% of the distance it leaves the closest
approaches significant well below their own value. All bodies here are small,
high-number objects with no measured size: the tabulated diameters are derived from
the absolute magnitude $H$ and an assumed (zone or default) albedo, not measured
...
\hline\hline
body 1 & body 2 & date (UTC) & dist (km) & $\sigma_d$ (km) & $v_{\rm rel}$ & $D_1$ & $D_2$ \\
       &        &            &           &                 & (km/s)        & (km)  & (km)  \\
\hline
(153222) 2000 YD43  & (238587) 2004 YX3   & 2016-03-11 & 1094 & 83 & 6.90 & 2.8 & 1.8 \\
(279617) 2011 EP37  & (219812) 2002 AM193 & 2016-06-16 & 1109 & 50 & 2.60 & 2.2 & 1.9 \\
(238813) 2005 NT70  & (391704) 2008 BB44  & 2017-03-13 & 1320 & 56 & 5.95 & 5.1 & 2.2 \\
(305032) 2007 TT418 & (393772) 2005 GV146 & 2016-07-12 & 1457 & 39 & 3.46 & 1.6 & 1.3 \\
(73282) 2002 JH61   & (261459) 2005 VP69  & 2015-04-01 & 1719 & 32 & 1.48 & 2.0 & 2.0 \\
\hline
\end{tabular}
\end{table*}
...
$\sim$1\,km against the $\sim$$5\times10^4$\,km approach, so unlike the sub-1000\,km
extreme approaches this separation is well determined; the caveat here is the proper
-vs-osculating element distinction, not the distance precision.
...
($D\approx240$\,km) with a close $3.2\times10^5$\,km approach. All ten already
...
\hline
(9) Metis       & 197 & MBA & 37 & $7.14\times10^5$ & 0.378 & $6.5\times10^{18}$ \\
(30) Urania     & 109 & --  & 36 & $4.40\times10^5$ & 0.817 & $8.2\times10^{17}$ \\
(40) Harmonia   & 141 & --  & 36 & $6.58\times10^5$ & 1.028 & $2.0\times10^{18}$ \\
(19) Fortuna    & 133 & MBA & 32 & $7.43\times10^5$ & 0.678 & $8.5\times10^{18}$ \\
(21) Lutetia    & 120 & --  & 30 & $8.18\times10^5$ & 0.609 & $1.7\times10^{18}$ \\
(64) Angelina   & 104 & MBA & 30 & $8.73\times10^5$ & 0.992 & --                 \\
(44) Nysa       & 140 & --  & 30 & $8.95\times10^5$ & 0.961 & $5.8\times10^{17}$ \\
(20) Massalia   & 178 & --  & 28 & $8.78\times10^5$ & 0.514 & $4.3\times10^{18}$ \\
(29) Amphitrite & 240 & --  & 27 & $3.17\times10^5$ & 0.643 & $1.4\times10^{19}$ \\
(27) Euterpe    & 141 & --  & 26 & $4.63\times10^5$ & 0.847 & $1.5\times10^{18}$ \\
\hline
\end{tabular}
\end{table*}
...
perturbers whose true mass is $\gtrsim2\times10^{19}$\,kg are recovered near
unity ratio, whereas the lighter ones are driven well below their true value by
the mass$\leftrightarrow$orbit regression (Sect.~below) and fall to bounds. The
clearest example is (88) Thisbe (FM mass $8.8\times10^{18}$\,kg, our fit
$6.9\times10^{18}$\,kg): a naive $\mathrm{snr_{jack}}$ flag would call it measured,
but the leverage-robust bootstrap $\sigma$ correctly returns it to a bound
(Sect.~below). The mass layer does not claim sensitivity below this frontier.
...
the correlated floor $s_c$ calibrated by bisection to $\chi^2_{\rm red} = 1$. This
...
under-powered: the total error bars of the non-calibrators (8--119\,\%) cannot
resolve deviations of the size the fits themselves show. Of the three measured
non-calibrators, all three lie \emph{below} the FM value with a geometric-mean ratio
of 0.85 --- a coherent $\sim$15\,\% one-sided pull (the regression-toward-zero
